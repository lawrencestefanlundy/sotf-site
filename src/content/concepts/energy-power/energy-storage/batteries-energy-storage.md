---
type: concept
slug: batteries-energy-storage
canonical_name: Batteries & Energy Storage
aliases: []
kind: technology
domain:
- cloudberry
- sotf
parent_concepts:
- energy-power
- energy-storage
related_concepts: []
companies_using:
- adden-energy
- addionics
- altris
- battolyser-systems
- catl-contemporary-amperex-technology-co
- cellforce-group
- cuberg-acquired-by-northvolt
- factorial-energy
- fluence-energy
- freyr-battery
- invinity-energy-systems
- lg-energy-solution
- lyten
- morrow-batteries
- northvolt-revolt-hydrovolt
- northvolt
- panasonic-energy
- quantumscape
- samsung-sdi
- solid-power
- storedot
- theion
- tiamat-energy
- verkor
- zenobe-energy
ideas_referencing: []
sources: []
frontier:
- ''
last_updated: '2026-05-04'
tags:
- concept
- technology
mention_count: 50
last_reorg_date: '2026-05-13'
sources_7d: 0
sources_30d: 0
---

# Batteries & Energy Storage

*Kind: technology · Attio deal count: 50*

## Physics / mechanism

Electrochemical energy storage converts chemical potential to electrical work via redox reactions at anode/cathode interfaces separated by an electrolyte. Key parameters: energy density (Wh/kg or Wh/L), power density (W/kg), cycle life, coulombic efficiency, and C-rate capability. Lithium-ion dominates at 250–300 Wh/kg cell-level; solid-state targets 400–500 Wh/kg with improved safety by replacing liquid electrolyte with ceramic or polymer. Flow batteries (vanadium, iron-air) decouple energy and power, suiting grid-scale. Sodium-ion is emerging for cost-sensitive stationary applications. Lifetime degradation mechanisms — lithium plating, SEI growth, cathode cracking — remain the core engineering constraint.

## Competitive landscape


Competing and adjacent approaches: supercapacitors offer 10–100× higher power density but 10–50× lower energy density, suited for buffer/regen applications. Hydrogen storage (compressed, liquid, solid-state metal hydride) competes at long-duration grid and heavy transport. Mechanical storage — pumped hydro, compressed air, flywheels — remains dominant in installed GWh terms. At materials level, cathode chemistry (NMC, LFP, NCA, LMFP) and anode choice (graphite vs. silicon vs. lithium metal) are the primary differentiation axes. Electrolyte innovation — sulfide vs. oxide solid electrolytes, ionic liquids — is where most deep-tech IP sits today.

| Technology | Energy Density | Best Use Case |
|---|---|---|
| Li-ion (NMC) | 250–300 Wh/kg | EV, portable |
| Vanadium flow | 20–40 Wh/kg | Long-duration grid |
| Solid-state Li | 400–500 Wh/kg (projected) | EV, aerospace |

## Cloudberry relevance
Cloudberry's angle is upstream: battery performance is gated by materials characterisation, deposition process control, and semiconductor-adjacent manufacturing — exactly where GlobalFoundries-LP expertise applies. Thin-film solid electrolyte deposition (ALD, PVD) maps directly to fab tooling. Deal-flow targets: solid-electrolyte IP spinouts, battery management IC designers, and sensor/photonics companies enabling in-situ cell diagnostics. Lunar Ventures' Berlin network surfaces European solid-state and sodium-ion seed rounds worth tracking.

## Companies using

<!-- dataview block stripped for public site -->

## Connected ideas

<!-- dataview block stripped for public site -->

## Sources

<!-- dataview block stripped for public site -->

## Frontier (open questions)

- *To be added.*
