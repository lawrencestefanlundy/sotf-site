...
No comparison is possible from the available sources. Adjacent approaches within the same taxonomy branch (generation, transmission, and market-facing "sell" layers) are documented elsewhere; the boundary question for storage is which duration bands it wins against firm generation and demand response, and no source here speaks to it.
