---
type: concept
slug: compound-semiconductors
canonical_name: Compound Semiconductors
aliases: []
kind: technology
domain:
- cloudberry
- sotf
parent_concepts:
- materials
- substrates
related_concepts: []
companies_using:
- azur-space-solar-power
- compound-semiconductor-centre-csc-sirio-semiconductor
- dynex-semiconductor
- efabless-xfab-x-fab-silicon-foundries
- epigan-imec-spin-off-now-soitec
- glo-x-formerly-glo-ab
- ii-vi-coherent-corp
- infineon-technologies
- iqe
- macom-technology-solutions
- navitas-semiconductor
- onsemi
- paragraf
- qorvo
- rockley-photonics
- rohm-semiconductor
- sanan-optoelectronics
- skyworks-solutions
- solaxis-swegan
- stmicroelectronics
- teledyne-e2v
- transphorm
- united-monolithic-semiconductors-ums
- win-semiconductors
- wolfspeed
ideas_referencing: []
sources:
- '[[2025-09-23-the-compute-gradient]]'
- '[[2026-02-12-how-to-invest-in-ai-sovereignty-sovereign]]'
- '[[2025-09-10-uk-opportunity-in-ai-compound-semiconductors]]'
- '[[2025-12-09-gallium-nitride-photonics-w-james]]'
frontier:
- ''
last_updated: '2026-05-04'
tags:
- concept
- technology
mention_count: 16
last_reorg_date: '2026-05-13'
sources_7d: 2
sources_30d: 2
---

# Compound Semiconductors

*Kind: technology · Attio deal count: 9*

## Physics / mechanism

Compound semiconductors are crystalline materials formed from two or more elements, typically III-V (GaAs, InP, GaN, InAs) or II-VI (CdTe, ZnSe) combinations. Unlike silicon, their direct bandgaps enable efficient photon emission and absorption; high electron mobility (GaAs ~8500 cm²/V·s vs Si ~1400) enables fast switching at lower voltages. Key parameters: bandgap tunability (0.17–3.4 eV across III-Vs), breakdown field (GaN ~3.3 MV/cm), lattice constant matching for epitaxial stack design. State of the art: 8-inch GaN-on-Si for power, InP-based PICs operating at 400G+, GaAs in 5G PA modules at >40% PAE. MOCVD and MBE dominate deposition; wafer costs remain 5–20× silicon equivalents.

## Competitive landscape


Silicon dominates logic and commodity analog but fails above ~1 keV RF frequencies, at high junction temperatures, and in photonic applications requiring direct bandgap. SiC competes with GaN in power switching (higher blocking voltage, better thermal conductivity, but lower frequency ceiling). Silicon photonics (SiPh) competes in datacom PICs via CMOS-compatible fab but struggles with efficient light emission and requires III-V heterogeneous integration for lasers. GaAs retains dominance in mobile PAs; InP leads coherent optical. Diamond and Ga₂O₃ are emerging ultra-wide-bandgap challengers, still pre-commercial.

| Material | Key strength | Key weakness |
|----------|-------------|--------------|
| GaN | RF + power, high temp | Substrate cost, defect density |
| InP | Coherent photonics, high-freq | Fragile, expensive, small wafers |
| SiC | Power, rugged | Limited RF, slower switching vs GaN |

## Cloudberry relevance
Cloudberry's GlobalFoundries-LP alignment is direct: GF operates compound semi capacity (GaN, SiGe BiCMOS) and compound semi IP is central to RF front-end, photonic integration, and power conversion—all active deal-flow verticals. Investable angles: epitaxy equipment (MOCVD tooling), substrate scaling (GaN-on-Si 8"), and fabless chipcos targeting datacom lasers or mmWave. Lunar Ventures' early-stage remit catches spinouts from Fraunhofer, imec, and III-V Lab before Series A. Key diligence flag: fab access and wafer-cost trajectory are often the real moat questions.

## Companies using

<!-- dataview block stripped for public site -->

## Connected ideas

<!-- dataview block stripped for public site -->

## Sources

<!-- dataview block stripped for public site -->

## Frontier (open questions)

- *To be added.*
